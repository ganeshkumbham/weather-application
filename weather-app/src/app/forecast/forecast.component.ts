  ...
  ngOnInit(): void {
    this.route.params.subscribe((params) => {
      if (params['zipcode']) {
        this.http
          .get<any>(
            `https://api.openweathermap.org/data/2.5/forecast?q=${params['zipcode']}&appid=5a4b2d457ecbef9eb2a71e480b947604`
          )
          .subscribe((data: any) => {
            this.APIData = data;
            this.APIData.list.forEach((element: any) => {
              if (this.finalArryList.length > 0) {
                let findData = this.finalArryList.find((d: any) => {
                  return (
                    moment(d.dt_txt).format('YYYY-MM-DD') ==
                    moment(element.dt_txt).format('YYYY-MM-DD')
                  );
                });
                if (!findData) {
