...
export class LandingScreenComponent implements OnInit {
  ...
  saveLocation() {
    ...
    if (this.locationArry && this.locationArry.length > 0) {
      ...
      this.http
        .get<any>(
          `https://api.openweathermap.org/data/2.5/weather?q=${this.locationId.value}&appid=5a4b2d457ecbef9eb2a71e480b947604`
        )
        .subscribe((data: any) => {
          APIData = data;
          this.locationArry.push({
            ZipCode: this.locationId.value,
            APIDatas: APIData,
          });
          localStorage.setItem(
            'Added_locations',
            JSON.stringify(this.locationArry)
          );
        });
    } else {
      this.locationArry = [];
  ...
  selectedData(obj: any) {
    this.findLoactionArry = localStorage.getItem('Added_locations');
    this.locationArry = JSON.parse(this.findLoactionArry);
    let findObj = this.locationArry.findIndex(
      (b: any) => b.ZipCode == obj.ZipCode
    );
